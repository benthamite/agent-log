...
*Stage 2: selection and narrative.*  The filtered session summaries (one-line and paragraph, from ~agent-log-summarize-sessions~) are sent to a selection model along with the original query.  This model produces a Markdown narrative explaining which sessions are relevant and why, with inline links to specific sessions.  The model and backend for this stage are configured via ~agent-log-search-model~ and ~agent-log-search-backend~ ([[#h:ai-search-options][AI search options]]).
