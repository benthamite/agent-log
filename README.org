...
Before starting, the command checks for sessions without current summaries.  If any exist, it warns the user that those sessions will be excluded and asks for confirmation.  If no sessions have current summaries at all, the command aborts with a message to run ~agent-log-summarize-sessions~ first ([[#h:summary-generation][Summary generation]]).
