...
~agent-log-redact-scrub-jsonl~ is the source-level scrubber.  It walks every directory in ~agent-log-redact-jsonl-roots~ recursively, and for every =.jsonl= older than ~agent-log-redact-active-session-seconds~ examines each line.  Lines that contain no pattern match are left verbatim (fast path); lines that do match are parsed as JSON, every string value redacted via ~agent-log-redact-text~, and the line re-serialized.  The file is rewritten atomically (temporary file + rename) only when at least one line actually changed.  The fast path is essential for idempotency: JSON parse + serialize is not guaranteed to be byte-identical for all valid inputs, so rewriting every line on every pass would cause spurious drift across re-runs.  Lines that fail to parse as JSON after matching a pattern are preserved unchanged, so malformed or truncated entries survive scrubbing.  The write path disables Emacs lockfiles so concurrent scrubber invocations do not deadlock on stale =.#= lock symlinks.  Run once after enabling redaction to clean historical sessions, and periodically afterward (manually, or via a cron-like schedule) to scrub newly-completed sessions.
