...
The Open column includes ~agent-log-resume-session~ (bound to =r=), which resumes the session displayed in the current buffer in the coding agent.  The Sync & AI column includes the ~agent-log-search~ command, bound to =/=.
