...
Before starting, the command checks index metadata without parsing session logs.  If any sessions lack current summaries, it warns the user that those sessions will be excluded and asks for confirmation.  If no sessions have searchable current summaries at all, the command aborts with a message to run ~agent-log-summarize-sessions~ first.  When existing summaries predate freshness tracking, the error says that those legacy summaries need refreshing rather than reporting them as never summarized.
