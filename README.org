...
2. /Source-level/ via ~agent-log-redact-scrub-jsonl~, which walks each .jsonl under ~agent-log-redact-jsonl-roots~ (Claude Code's =~/.claude/projects/=, Codex's =~/.codex/sessions/=), parses each line as JSON, and rewrites every string value through ~agent-log-redact-text~ atomically.  JSON object keys are preserved unchanged.  Files modified within ~agent-log-redact-active-session-seconds~ are skipped to avoid racing with an active session.

Source-level scrubbing is the authoritative redaction: once the JSONL is clean, every downstream rendering of that session is automatically clean, and there is no plain-text copy of the secret on local disk.  Session-resume still works — a resumed conversation sees =[REDACTED:label:hash]= strings in prior history, which any coding agent handles as normal text.
