...
Before starting, the command validates and upgrades compatible legacy summaries using the same fingerprint check as ~agent-log-summarize-sessions~ ([[#h:summary-generation][Summary generation]]).  It then checks for sessions without current summaries.  If any exist, it warns the user that those sessions will be excluded and asks for confirmation.  If no sessions have searchable current summaries at all, the command aborts with a message to run ~agent-log-summarize-sessions~ first.  When existing summaries predate freshness tracking and cannot be upgraded, the error says that those legacy summaries need refreshing rather than reporting them as never summarized.
...
Each =.md= file contains HTML-comment front matter (session ID, source path, rendered timestamp, JSONL size) followed by the full conversation rendered as Markdown.  The =_index.el= file is a hash table mapping session IDs to metadata (rendered file path, JSONL size, and optionally a summary plus the conversation fingerprint and JSONL file state it describes).  Summary commands use that metadata to distinguish current summaries, legacy summaries that can be upgraded without another LLM request, and stale summaries that need to be regenerated.
