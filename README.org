...
Summaries are persisted in the index file together with a fingerprint of the conversation text they describe, so they survive across Emacs sessions and are refreshed only when the underlying conversation changes.  If you resume a previously summarized session and continue working, the next summarization run treats the old summary as stale and generates a replacement.  Legacy summaries created before this fingerprint existed are refreshed once.  When you open a session that has a current summary, the paragraph summary is inserted as a blockquote below the session header.
