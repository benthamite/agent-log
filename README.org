...
Redaction is applied via =:filter-return= advice on ~agent-log--render-entry~, so it covers both the full-render path (~agent-log--render-to-file~) and the live-update path (~agent-log--append-rendered-line~).  Source JSONL files are not touched — they remain in plain text at each agent's own location (for Claude Code, under =~/.claude/projects/=) and should be protected with filesystem-level controls.  This is by design: rewriting the JSONL would risk breaking session resumption.

Redaction covers the rendered Markdown that you actually search and browse; it is not a substitute for preventing secrets from entering the transcript in the first place.
