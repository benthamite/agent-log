...
Summaries are persisted in the index file together with a fingerprint and source file state for the conversation text they describe, so they survive across Emacs sessions and are refreshed only when the underlying conversation changes.  If you resume a previously summarized session and continue working, the next summarization run treats the old summary as stale and generates a replacement.  Legacy summaries created before this freshness metadata existed are refreshed once.  When you open a session that has a current summary, the paragraph summary is inserted as a blockquote below the session header.
...
Before starting, the command checks for sessions without current summaries.  If any exist, it warns the user that those sessions will be excluded and asks for confirmation.  If no sessions have searchable current summaries at all, the command aborts with a message to run ~agent-log-summarize-sessions~ first ([[#h:summary-generation][Summary generation]]).  When existing summaries predate freshness tracking, the error says that those legacy summaries need refreshing rather than reporting them as never summarized.
