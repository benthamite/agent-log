...
*Stage 2: selection and narrative.*  The filtered session summaries (one-line and paragraph, from ~agent-log-summarize-sessions~) are sent to a selection model along with the original query.  The prompt includes a human-readable description of the scope applied in stage 1 (projects, date range, and how many sessions were included out of the total).  The model produces a Markdown narrative explaining which sessions are relevant and why, with inline links to specific sessions.  If the scope is narrower than "all projects, all dates", the model notes this so you know some sessions may have been excluded.  The model and backend for this stage are configured via ~agent-log-search-model~ and ~agent-log-search-backend~ ([[#h:ai-search-options][AI search options]]).
