...
~agent-log-redact-scrub-jsonl~ is the source-level scrubber.  It walks every directory in ~agent-log-redact-jsonl-roots~ recursively, and for every =.jsonl= older than ~agent-log-redact-active-session-seconds~ parses each line as JSON, redacts every string value via ~agent-log-redact-text~, and rewrites the file atomically (temporary file + rename) when anything changed.  Lines that fail to parse as JSON are preserved unchanged, so malformed or truncated entries survive scrubbing.  The write path disables Emacs lockfiles so concurrent scrubber invocations do not deadlock on stale =.#= lock symlinks.  Run once after enabling redaction to clean historical sessions, and periodically afterward (manually, or via a cron-like schedule) to scrub newly-completed sessions.
